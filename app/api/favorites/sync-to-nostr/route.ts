...
export async function GET(request: NextRequest) {
  try {
    ...
    // Get album favorites (exclude publisher favorites from NIP-51 republish):
    // - If force=all: get ALL non-publisher favorites (complete re-sync)
    // - If force=true: get non-publisher ones needing NIP-51 republishing
    // - Otherwise: get unpublished ones
    const albumWhere: any = { userId };
    if (forceAll) {
      albumWhere.type = { not: 'publisher' };
    } else if (forceNip51) {
